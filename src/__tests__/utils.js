...
test('resolveRwScripts resolves to src/index.js when in the rw-scripts package', () => {
  mockPkg({package: {name: 'rw-scripts'}});
  expect(require('../utils').resolveRwScripts()).toBe(
      require.resolve('../').replace(process.cwd(), '.'),
  );
});

test('resolveRwScripts resolves to rw-scripts if not in the rw-scripts package', () => {
  mockPkg({package: {name: 'not-rw-scripts'}});
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'));
  expect(require('../utils').resolveRwScripts()).toBe('rw-scripts');
});
